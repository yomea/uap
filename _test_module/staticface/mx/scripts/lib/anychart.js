...
AnyChart = com.anychart.AnyChart;
...
AnyChart.__disposeInIE = function()
{
	if (AnyChart) {
        if (AnyChart.utils && AnyChart.utils.listeners) {
            for (var i = 0, len = AnyChart.utils.listeners.length; i < len; i++)
                AnyChart.utils.listeners[i][0].detachEvent(AnyChart.utils.listeners[i][1], AnyChart.utils.listeners[i][2]);
        }
        if (AnyChart.charts) {
        	try{
        		while (AnyChart.charts.length > 0)
            	{
            		AnyChart.charts[AnyChart.charts.length - 1].dispose();
            	}
        	}catch(e){
        	}
        }

        for (var i in AnyChart)
            AnyChart[i] = null;
    }
...
//Commented by tiger on 20140327
...
AnyChart.prototype = {
    ...
    dispose: function() {
    	this._onChartLoad = null;
    	this._onHTMLCreate = null;
        this.remove();
    },

    removeFlashObject: function() {
        if (this.flashObject) {
            if (this.flashObject.Dispose){
            	this.flashObject.Dispose();
            } 
    ...
    setData: function(data) {
        ...
        if (this._isXMLData(data))
            this._xmlSource = String(data);
        else
            this._xmlSource = AnyChart.utils.JSConverter.convert(data);

        if (this._isChartCreated)
            this._execSetXMLData();
        else
            this._needSetXMLDataAfterCreation = this._xmlSource != null;
    },
    ...
    _execSetXMLData: function() {
        if (this._xmlSource != null && this.flashObject && this.flashObject.SetXMLDataFromString)
            this.flashObject.SetXMLDataFromString(this._xmlSource);
    },
